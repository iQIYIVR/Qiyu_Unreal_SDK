	...
	FClearValueBinding::White)
{
	...
	for (int i = 0; i < CVars::GNumSwapchainImages; ++i)
	{
		...
		if ((InTargetableTextureFlags & TexCreate_RenderTargetable) && InFormat == PF_B8G8R8A8 && !FOpenGL::SupportsBGRA8888RenderTarget())
		{
		...
		if (FOpenGL::SupportsTextureSwizzle() && GLFormat.bBGRA && !(InTargetableTextureFlags & TexCreate_RenderTargetable))
		{
			glTexParameteri(InTarget, GL_TEXTURE_SWIZZLE_R, GL_BLUE);
			glTexParameteri(InTarget, GL_TEXTURE_SWIZZLE_B, GL_RED);
		}

		if (GetMutableDefault<UQiyuSettings>()->foveationFeature != 0)
		{
			...
			checkf(GetMutableDefault<UQiyuSettings>()->lowFoveationMinimum >= 0.f && GetMutableDefault<UQiyuSettings>()->lowFoveationMinimum <= 1.f,
				TEXT("GL_QCOM_texture_foveated: The minimum pixel density (%f), should be [0.f;1.f]!"), GetMutableDefault<UQiyuSettings>()->lowFoveationMinimum);

			glTexParameterf(InTarget, GL_TEXTURE_FOVEATED_MIN_PIXEL_DENSITY_QCOM, GetMutableDefault<UQiyuSettings>()->lowFoveationMinimum);

			dbgprint("GL_QCOM_texture_foveated: Minimum pixel density = %f", GetMutableDefault<UQiyuSettings>()->lowFoveationMinimum);
		}
