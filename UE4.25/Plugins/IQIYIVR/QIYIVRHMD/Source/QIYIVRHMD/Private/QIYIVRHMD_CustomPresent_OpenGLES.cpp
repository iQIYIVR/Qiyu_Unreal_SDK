	...
	uint32 NumSamples)
{
	...
	{
		GetMutableDefault<UQiyuSettings>()->foveationFeature = 0;
	}
...
void FOpenGLESCustomPresentSVR::SubmitFrame(const FPoseStateFrame& PoseState)
{
	...
	if (GetMutableDefault<UQiyuSettings>()->foveationFeature != 0 || CVars::EyeMarkerEnabled != 0)
	{
...
#undef AppendBitFieldName
	...
	//UE_LOG(LogSVR, Log, TEXT("Qiyu FoveationTexture foveationFeature=%d  bFoveationEnabled=%d"),
	//	GetMutableDefault<UQiyuSettings>()->foveationFeature,
	//	GetMutableDefault<UQiyuSettings>()->bFoveationEnabled
	//);
	//UE_LOG(LogSVR, Log, TEXT("Qiyu FoveationTexture FoveationArea=%f FoveationGain(%f,%f) LowFoveationMinimum=%f"),
	//		GetMutableDefault<UQiyuSettings>()->FoveationArea,
	//		GetMutableDefault<UQiyuSettings>()->FoveationGain.X,
	//		GetMutableDefault<UQiyuSettings>()->FoveationGain.Y,
	//		GetMutableDefault<UQiyuSettings>()->lowFoveationMinimum
	//		);

	if (GetMutableDefault<UQiyuSettings>()->foveationFeature != 0 && GetMutableDefault<UQiyuSettings>()->bFoveationEnabled != 0)
	{
		if (UsesArrayTexture())
		{
			...
			glTextureFoveationParametersQCOM(TextureHandle,         // texture
				...
				GetMutableDefault<UQiyuSettings>()->FoveationGain.X, // gainX
				GetMutableDefault<UQiyuSettings>()->FoveationGain.Y,	// gainY
				GetMutableDefault<UQiyuSettings>()->FoveationArea	// foveaArea
			);
		...
		else
		{
			...
			glTextureFoveationParametersQCOM(TextureHandle,						// texture
											 ...
											 FocusPoints[0].Y,					// focalY
				GetMutableDefault<UQiyuSettings>()->FoveationGain.X * 2.f,		// gainX
				GetMutableDefault<UQiyuSettings>()->FoveationGain.Y,				// gainY
				GetMutableDefault<UQiyuSettings>()->FoveationArea		// foveaArea
			);
