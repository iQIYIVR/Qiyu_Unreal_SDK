...
void QIYIVRSdkCore_RequestMessageCallBack(QiyiVRCore::uint64 messageID, QiyiVRCore::PCChar result, QiyiVRCore::int32 resultSize)
{
	UQIYIVRDataManager::GetInstance()->MessageProcess(messageID, result, resultSize);
}

void QIYIVRSdkCore_ListenerMessageCallBack(QiyiVRCore::uint64 messageID, QiyiVRCore::PCChar result, QiyiVRCore::int32 resultSize)
{
...
bool FQIYIVRHMD::OnStartGameFrame( FWorldContext& WorldContext )
{
    ...
    if (bResumed && 
        ...
        bInitialized)
    {
		UE_LOG(LogSVR, Log, TEXT("FQIYIVRHMD::OnStartGameFrame() - BeginVRMode 22222222"));
		if (count >= 50)
			BeginVRMode();
...
void FQIYIVRHMD::CalculateRenderTargetSize(const class FViewport& Viewport, uint32& InOutSizeX, uint32& InOutSizeY)
{
    ...
    InOutSizeX = di.targetEyeWidthPixels * ScalingFactor;
	...
	//}
	UE_LOG(LogSVR, Log, TEXT("InOutSizeX: %d,  InOutSizeY: %d"), InOutSizeX, InOutSizeY);
#endif
}
...
bool FQIYIVRHMD::AllocateRenderTargetTexture(uint32 Index, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, ETextureCreateFlags InFlags, ETextureCreateFlags TargetableTextureFlags, FTexture2DRHIRef& OutTargetableTexture, FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
#else
bool FQIYIVRHMD::AllocateRenderTargetTexture(uint32 Index, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, uint32 InFlags, uint32 TargetableTextureFlags, FTexture2DRHIRef& OutTargetableTexture, FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
#endif
{
